### 📂 `.agent\skills\nibandha-manager\resources\recovery_protocols.md`

# Quality Manager Recovery Protocols: The "Self-Healing" Manual

This document defines how the Quality Manager reacts when `scripts/check_foundations.py` or `scripts/project_doctor.py` reports a failure.
...
3. **Instruction to User:** "Please run environment setup. Quality Manager cannot operate outside the locked .venv."
...
**Detection:** Conversation length causes the agent to ignore the `NIBANDHA_ARCH.md` standards.

* **Instruction:** **State Refresh.**
* **Action:** 1. Re-read `resources/NIBANDHA_ARCH.md`.
