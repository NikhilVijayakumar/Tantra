...
The TANTRA ecosystem follows the `MAJOR.MINOR.PATCH` format. The **Package-Maintainer** must use this guide to determine the next version in `pyproject.toml`.
...
```python
# src/bavans/tantra/__init__.py
from importlib.metadata import version, PackageNotFoundError

try:
    __version__ = version("TANTRA")
